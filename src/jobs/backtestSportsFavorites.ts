...
interface Config {
  ...
  maxMarkets: number;
  entryMinutesBeforeStart: number;
  priceLookbackHours: number;
...
function configFromEnv(): Config {
  ...
  if (!Number.isFinite(start.getTime()) || !Number.isFinite(end.getTime()) || start >= end) {
    throw new Error("Invalid SPORTS_BACKTEST_START/END");
  }
  return {
...
function normalizeMarket(raw: GammaMarketRaw, sourceSport: string, config: Config): CandidateMarket | null {
  ...
  const sport = classifySport(slug, sourceSport);
  if (!sport || !config.sports.includes(sport)) return null;

  const outcomes = parseStringList(raw.outcomes);
  const outcomePrices = parseStringList(raw.outcomePrices).map(Number);
  const tokenIds = parseStringList(raw.clobTokenIds);
  if (outcomes.length !== 2 || outcomePrices.length !== 2 || tokenIds.length !== 2) return null;
  if (!isMoneyline(raw, outcomes)) return null;
  const winningIndex = parseTerminalWinner(outcomePrices);
  if (winningIndex == null) return null;

  const eventStartTime = raw.eventStartTime ?? raw.gameStartTime;
  if (!eventStartTime) return null;
  const eventStart = new Date(eventStartTime);
  if (!Number.isFinite(eventStart.getTime()) || eventStart < config.start || eventStart > config.end) return null;

  const tick = Number(raw.orderPriceMinTickSize ?? 0.01);
  const tickSize = Number.isFinite(tick) && tick > 0 ? tick : 0.01;
  const feeRate = normalizeFeeRate(raw.feeSchedule?.rate, 0.03);
  return {
    sport,
    id,
    conditionId,
...
async function fetchClosedMarketsForTag(tagId: string, sourceSport: string, config: Config): Promise<CandidateMarket[]> {
  ...
  for (let offset = 0; offset <= 10_000; offset += PAGE_SIZE) {
    ...
    for (const raw of page) {
      const market = normalizeMarket(raw, sourceSport, config);
...
async function fetchTradeWindow(conditionId: string, start: number, end: number, depth = 0): Promise<HistoricalTrade[]> {
  const query = new URLSearchParams({
    market: conditionId,
...
function rowsToCsv(rows: BacktestRow[]): string {
  const keys = Object.keys(rows[0] ?? {}) as (keyof BacktestRow)[];
...
function printSummary(label: string, rows: BacktestRow[]): void {
  ...
  console.log(
    `${label.padEnd(12)} n=${String(summary.n).padStart(3)} ` +
    `win=${formatPct(summary.winRate).padStart(7)} ` +
    `ROI=${formatPct(summary.roi).padStart(8)} ` +
    `PnL=$${summary.pnl.toFixed(2).padStart(8)} ` +
    `CI=[${formatPct(summary.ci95Low)}, ${formatPct(summary.ci95High)}]`,
  );
}
...
async function main(): Promise<void> {
  ...
  for (const wanted of config.sports) {
    ...
    for (const row of matching) {
      for (const tagId of parseStringList(row.tags)) tagSources.push({ tagId, sport: wanted });
    }
  }
  ...
  // One independent moneyline per game. Never choose by final volume or outcome.
  const byGame = new Map<string, CandidateMarket>();
  for (const market of eligible) {
    const key = market.gameId ?? `${market.sport}|${market.eventStartTime}|${market.slug}`;
    if (!byGame.has(key)) byGame.set(key, market);
  }
  ...
  const processed = await mapLimit(selected, config.concurrency, async (market): Promise<BacktestRow | null> => {
    try {
      const gameStartTs = Math.floor(Date.parse(market.eventStartTime) / 1000);
      const entryTs = gameStartTs - config.entryMinutesBeforeStart * 60;
      const windowStart = entryTs - config.priceLookbackHours * 3600;
      const windowEnd = entryTs + config.fillWindowSeconds;
      const trades = await fetchTradeWindow(market.conditionId, windowStart, windowEnd);
      const favorite = favoriteAtEntry(trades, market.tokenIds, entryTs, config.maxPriceAgeMinutes * 60);
      if (!favorite) return skip("no_recent_two_sided_price");

      const fill = simulateTakerBuy(
        trades,
        favorite.tokenId,
        entryTs,
        config.fillWindowSeconds,
        config.budget,
        market.feeRate,
        market.tickSize,
      );
      if (!fill) return skip("no_buy_print_after_entry");
      if (fill.fillRatio < config.minFillRatio) return skip("insufficient_fill");
      if (fill.allInPrice < 0.65 || fill.allInPrice > 0.80) return skip("outside_65_80");

      const won = favorite.outcomeIndex === market.winningIndex;
      const payout = won ? fill.shares : 0;
      const pnl = payout - fill.cashSpent;
      return {
        sport: market.sport,
        marketId: market.id,
        ...
        favoriteOutcome: market.outcomes[favorite.outcomeIndex],
        favoriteTokenId: favorite.tokenId,
        referencePrice: favorite.referencePrice,
        averageFillPrice: fill.averagePrice,
        allInPrice: fill.allInPrice,
        feePaid: fill.feePaid,
        shares: fill.shares,
        cashSpent: fill.cashSpent,
        fillRatio: fill.fillRatio,
        fillSeconds: fill.fillSeconds,
        won,
        pnl,
        roi: pnl / fill.cashSpent,
      };
    } catch (error) {
      console.warn(`SKIP ${market.slug}: ${(error as Error).message}`);
      return skip("api_error");
    } finally {
      completed++;
      if (completed % 25 === 0 || completed === selected.length) console.log(`Processed ${completed}/${selected.length}`);
    }
  });
  ...
  const developmentCount = Math.floor(rows.length * 0.7);
  const development = rows.slice(0, developmentCount);
  const holdout = rows.slice(developmentCount);
  const coverage = selected.length ? rows.length / selected.length : 0;

  console.log("\n=== RESULTS ===");
  printSummary("ALL", rows);
  printSummary("DEVELOPMENT", development);
  printSummary("HOLDOUT", holdout);
  for (const sport of config.sports) printSummary(sport.toUpperCase(), rows.filter((row) => row.sport === sport));
  for (const [lo, hi] of [[0.65, 0.70], [0.70, 0.75], [0.75, 0.800001]]) {
    printSummary(`${lo.toFixed(2)}-${Math.min(hi, 0.80).toFixed(2)}`, rows.filter((row) => row.allInPrice >= lo && row.allInPrice < hi));
  }
  console.log(`Coverage: ${rows.length}/${selected.length} = ${formatPct(coverage)}`);
  console.log("Skips:", Object.fromEntries([...skipCounts.entries()].sort((a, b) => b[1] - a[1])));
  ...
  const report = {
    ...
    methodology: {
      ...
      limitations: [
        ...
        "unfilled markets are skipped and coverage is reported",
      ],
    },
    ...
    summaries: {
      all: summarizeBacktest(rows),
      development: summarizeBacktest(development),
  ...
  const base = path.join(directory, `sports-favorites-${stamp}`);
  await Promise.all([
    writeFile(`${base}.json`, JSON.stringify(report, null, 2), "utf8"),
    writeFile(`${base}.csv`, rowsToCsv(rows), "utf8"),
  ]);
