...
interface BacktestRowWithSource extends BacktestRow {
...
function isMoneyline(raw: GammaMarketRaw, outcomes: string[]): boolean {
  ...
  const text = (raw.slug ?? "") + " " + (raw.question ?? "");
...
function normalizeMarket(raw: GammaMarketRaw, sourceSport: string, config: Config): CandidateMarket | null {
  ...
  return { sport, id, conditionId, slug, question: String(raw.question ?? ""), gameId: raw.gameId ? String(raw.gameId) : null,
    ...
    feeRate: normalizeFeeRate(raw.feeSchedule?.rate, 0.10) };
}
async function fetchClosedMarketsForTag(tagId: string, sourceSport: string, config: Config): Promise<CandidateMarket[]> {
  ...
  for (let offset = 0; offset <= 10_000; offset += PAGE_SIZE) {
    const qs = new URLSearchParams({ closed: "true", limit: String(PAGE_SIZE), offset: String(offset), tag_id: tagId,
      related_tags: "true", end_date_min: config.start.toISOString(), end_date_max: config.end.toISOString() });
    let page: GammaMarketRaw[];
    try {
      page = await fetchJson<GammaMarketRaw[]>(GAMMA_API + "/markets?" + qs);
    } catch {
      break;
    }
...
async function fetchTradeWindow(conditionId: string, start: number, end: number): Promise<HistoricalTrade[]> {
  const fetchPage = async (offset: number): Promise<DataTradeRaw[]> => {
    const qs = new URLSearchParams({ market: conditionId, limit: String(TRADE_LIMIT), offset: String(offset), takerOnly: "true" });
    const raw = await fetchJson<DataTradeRaw[]>(DATA_API + "/trades?" + qs);
...
function rowsToCsv(rows: BacktestRowWithSource[]): string {
  const keys = Object.keys(rows[0] ?? {}) as (keyof BacktestRowWithSource)[]; if (!keys.length) return "";
...
async function main(): Promise<void> {
  ...
  let completed = 0;
  const processed = await mapLimit(selected, config.concurrency, async (market): Promise<BacktestRowWithSource | null> => {
    try {
      ...
      const rangeStart = entryTs - config.priceLookbackHours * 3600;
      const rangeEnd = entryTs + config.fillWindowSeconds;
      const trades = await fetchTradeWindow(market.conditionId, rangeStart, rangeEnd);
      ...
      if (fill) {
        ...
        return { sport: market.sport, marketId: market.id, conditionId: market.conditionId, slug: market.slug, gameId: market.gameId,
          ...
          fillSource: "taker_print" as const };
      }
      // synthetic fill when no taker prints but we have a reference price
      const syntheticPrice = Math.min(0.9999, favorite.referencePrice + market.tickSize);
      const feePerShare = takerFeePerShare(syntheticPrice, market.feeRate);
      const allInPrice = syntheticPrice + feePerShare;
      if (allInPrice < 0.65 || allInPrice > 0.80) return skip("outside_65_80_synthetic");
      const synthShares = config.budget / allInPrice;
      const synthCashSpent = synthShares * allInPrice;
      const synthFeePaid = synthShares * feePerShare;
      const won = favorite.outcomeIndex === market.winningIndex;
      const payout = won ? synthShares : 0;
      const pnl = payout - synthCashSpent;
      return { sport: market.sport, marketId: market.id, conditionId: market.conditionId, slug: market.slug, gameId: market.gameId,
        eventStartTime: market.eventStartTime, favoriteOutcome: market.outcomes[favorite.outcomeIndex], favoriteTokenId: favorite.tokenId,
        referencePrice: favorite.referencePrice, averageFillPrice: syntheticPrice, allInPrice, feePaid: synthFeePaid,
        shares: synthShares, cashSpent: synthCashSpent, fillRatio: 1, fillSeconds: 0, won, pnl, roi: pnl / synthCashSpent,
        fillSource: "synthetic" as const };
  ...
  const rows = processed.filter((row): row is BacktestRowWithSource => row != null).sort((a, b) => Date.parse(a.eventStartTime) - Date.parse(b.eventStartTime));
  ...
  printSummary("TAKER_PRINT", takerRows);
  printSummary("SYNTHETIC", synthRows);
  console.log("Taker print: " + takerRows.length + " trades, PnL=$" + takerRows.reduce((s,r) => s + r.pnl, 0).toFixed(2));
  console.log("Synthetic:   " + synthRows.length + " trades, PnL=$" + synthRows.reduce((s,r) => s + r.pnl, 0).toFixed(2));
  ...
  const report = { generatedAt: new Date().toISOString(), methodology: {
    ...
    execution: "subsequent taker BUY prints for " + config.fillWindowSeconds + "s, one extra tick slippage, market feeSchedule when present; synthetic fill when no prints found",
    limitations: ["Data API exposes at most 20,000 market trades; denser markets fail closed",
        "trade prints do not reconstruct historical resting order-book depth",
        "tests direct pregame favorite bias, not wallet-selection alpha",
        "synthetic fills assume immediate fill at referencePrice+tick with no queue"] },
    config: { ...config, start: config.start.toISOString(), end: config.end.toISOString() },
    counts: { discovered: discovered.length, eligible: eligible.length, independent: independent.length,
      selected: selected.length, executed: rows.length, coverage: coverage,
      takerPrint: takerRows.length, synthetic: synthRows.length,
      skips: Object.fromEntries(skipCounts) },
    summaries: { all: summarizeBacktest(rows), development: summarizeBacktest(development),
      holdout: holdoutSummary, takerPrint: summarizeBacktest(takerRows),
      synthetic: summarizeBacktest(synthRows) },
